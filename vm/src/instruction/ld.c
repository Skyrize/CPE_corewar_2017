...
void assign_new_value_to_new_registre(int new_num, int num_of_registre, champ_t *champ, pc_t *pc)
{
...
void read_t_dir_ld(byte *tab, pc_t *pc, champ_t *champ)
{
	int get_num = get_short_int(tab + pc->idx + 1);
	int new_num = get_short_int(tab + (pc->idx + get_num % IDX_MOD));
	int get_registre = my_getnbr(tab + pc->idx + 2);
	assign_new_value_to_new_registre(new_num, get_registre);
}

void read_t_ind_ld(byte *tab, pc_t *pc, champ_t *champ)
{
